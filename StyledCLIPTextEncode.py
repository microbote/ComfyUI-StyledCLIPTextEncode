...
styleNames = read_sdxl_styles(sdxl_styles)


# 定义新的节点类
class StyledCLIPTextEncode(CLIPTextEncode):
    def __init__(self) -> None:
        super().__init__()

    @classmethod
    def getStyleList(cls):
        global styleNames
        return styleNames

    @classmethod
    def createPositivePrompt(cls, select_style, positive_prompt):
      try:
        for template in sdxl_styles:
        ...
        raise ValueError(f"No template found with name '{select_style}'.")

      except Exception as e:
        print(f"An error occurred: {str(e)}")

    @classmethod
    def createNegativePrompt(cls, select_style, negative_prompt):
      try:
        for template in sdxl_styles:
          # Check if template contains 'name' and 'prompt' fields
          if 'name' not in template or 'prompt' not in template:
          ...
          if template['name'] == select_style:
            ...
            if negative_prompt:
              negative = f"{json_negative_prompt}, {negative_prompt}" if json_negative_prompt else negative_prompt
            else:
    ...
    RETURN_TYPES = ("CONDITIONING", "CONDITIONING")  # positive, negative conditioning
    FUNCTION = "encodeit"
    CATEGORY = "conditioning"

    def encodeit(self, clip, positive_prompt, negative_prompt, select_style):
        positive_cond = super().encode(clip, self.createPositivePrompt(select_style, positive_prompt))[0]
        negative_cond = super().encode(clip, self.createNegativePrompt(select_style, negative_prompt))[0]
...
NODE_DISPLAY_NAME_MAPPINGS = {
    "StyledCLIPTextEncode": "Styled CLIP Text Encode (Prompt)",
}
